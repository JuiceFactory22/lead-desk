...
- `GHL_PRIVATE_TOKEN` / `GHL_LOCATION_ID` — from GoHighLevel's Private
  Integrations settings (Settings > Integrations > Private Integrations)
- `GHL_FALLBACK_NUMBER` — last-resort sending number, only used if no
  territory matches and none is marked default (see below)
...
address, job details) via GoHighLevel, and marks the claim "delivered" — no
...
**If the automatic text fails** (bad number, GHL outage, etc.) — the
...
either, to `GHL_FALLBACK_NUMBER` so delivery never just breaks — but
...
- **Moving the opt-in text too** ("want this lead?") off CallRail and onto
  GoHighLevel, now that GHL is already doing the automatic-delivery half.
  GHL could run that outbound blast and call a webhook back into this app
  instead of requiring the "Mark interested" click — this would replace
  the one remaining manual step with an automated one.
